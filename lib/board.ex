defmodule Board do
  ...
  def get_cell_line_to_print(cell_level, at_end_of_row?, value) do
    line_to_print = case cell_level do
      "top" -> "#{value}    "
      "middle" -> "  #{value}  "
      "bottom" -> "_____"
      "bottom_of_last_row" -> "     "
    ...
    if at_end_of_row?, do: line_to_print, else: line_to_print <> "|"
  end
  ...
  def print_cell_line(cell_level, num_of_columns, row_idx, board_data) do
    Enum.reduce(1..num_of_columns, "", fn (column_idx, acc) ->
      ...
      cell_value = if cell_level == "middle", do: Map.get(board_data, curr_column_number), else: curr_column_number
  ...
  def print_cell_row(num_of_columns, num_of_rows, row_idx, board_data) do
    ...
    last_line = case num_of_rows - 1 == row_idx do
